...
export default function Messages() {
  ...
  if (!role) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-bold text-slate-700 mb-2">Login Required</h2>
        <p className="text-slate-500 max-w-sm">You need to log in to access the messaging system.</p>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-24">
      ...
      <div className="bg-white border border-slate-200 rounded-[28px] overflow-hidden shadow-sm flex flex-col h-[60vh] min-h-[500px]">
        {isCoreOrAdmin && (
          <div className="flex p-2 gap-2 bg-slate-50 border-b border-slate-200">
            <button
              onClick={() => setActiveTab('student')}
              className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${
                activeTab === 'student' ? 'bg-white shadow-sm text-slate-900 border border-slate-200' : 'text-slate-500 hover:bg-slate-100'
              }`}
            >
              Student Chat
            </button>
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${
                activeTab === 'core' ? 'bg-[#2563eb] shadow-sm text-white' : 'text-slate-500 hover:bg-slate-100'
              }`}
            >
              Core Team Chat
            </button>
        ...
        <div className="p-4 border-b border-slate-100 bg-white">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm focus:border-[#2563eb] outline-none transition-colors"
            />
          </div>
        </div>

        <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-slate-700 font-bold mb-1">Select a contact to start chatting</h3>
