...
export default function Home() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in duration-500">
      <div className="flex bg-[#1e293b] p-1 rounded-xl md:rounded-2xl border border-slate-700 backdrop-blur-md relative overflow-x-auto scrollbar-hide shadow-sm">
  ...
  return (
    <button
      ...
      className={`flex-1 min-w-[100px] py-2.5 md:py-3 px-2 flex items-center justify-center gap-1.5 md:gap-2 rounded-lg md:rounded-xl text-[11px] md:text-sm font-bold transition-all duration-300 relative z-10 ${
        active ? 'bg-gradient-to-r from-[#2563eb] to-[#3b82f6] text-white shadow-[0_4px_20px_rgba(37,99,235,0.3)]' : 'text-slate-400 hover:text-white hover:bg-slate-800'
      }`}
    >
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-[#1e293b] border border-slate-700 p-6 rounded-[24px] text-center relative overflow-hidden group shadow-sm">
          <div className="absolute top-0 right-0 p-4 opacity-[0.03]">
            <span className="text-8xl">🏆</span>
          </div>
          <p className="text-xs text-slate-400 mb-2 font-bold tracking-widest uppercase relative z-10">Active Events</p>
        ...
        <div className="bg-[#1e293b] border border-slate-700 p-6 rounded-[24px] text-center relative overflow-hidden group shadow-sm">
          ...
          <p className="text-xs text-slate-400 mb-2 font-bold tracking-widest uppercase relative z-10">Committee</p>
          <h2 className="text-4xl md:text-5xl font-extrabold text-[#2563eb] relative z-10">{coreMembers.length}</h2>
        </div>
      </div>
      
      <div className="bg-[#1e293b] p-6 rounded-[28px] border border-slate-700 shadow-sm">
            ...
            {canEditHolidays && (
              <div className="flex gap-2 shrink-0">
                ...
                <button 
                  ...
                  className={`p-2 rounded-xl transition-colors text-slate-400 hover:text-white flex items-center gap-2 text-xs font-bold ${holidayPdf ? 'bg-slate-700 hover:bg-slate-200' : 'bg-[#2563eb]/10 text-[#2563eb] hover:bg-[#2563eb]/20 hover:text-[#1d4ed8]'}`} 
                  title={holidayPdf ? "Replace PDF" : "Upload PDF"}
                >
                  <Upload size={16} /> {holidayPdf ? '' : 'Upload'}
                </button>
                <button onClick={handleAddHoliday} className="p-2 bg-slate-700 rounded-xl hover:bg-[#2563eb] hover:text-white transition-colors text-slate-300">
                  <Plus size={16} />
                </button>
              </div>
            )}
        ...
        <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
          {holidays.map(h => (
            <div key={h.id} className="min-w-[180px] p-5 rounded-2xl bg-[#1e293b] border border-slate-700 shadow-sm shrink-0 relative group">
              <p className="text-[10px] font-bold text-[#2563eb] mb-2 tracking-wider uppercase">{h.dateRange}</p>
              <p className="font-bold text-lg leading-tight text-white">{h.title}</p>
              {canEditHolidays && (
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => handleEditHoliday(h)} className="p-1 bg-slate-700 rounded-lg text-slate-400 hover:text-[#2563eb]">
                    <Edit2 size={10} />
                  </button>
                  <button onClick={() => deleteHoliday(h.id)} className="p-1 bg-slate-700 rounded-lg text-slate-400 hover:text-red-500">
                    <Trash2 size={10} />
                  </button>
                </div>
              )}
      ...
      <div className="bg-[#1e293b] p-6 rounded-[28px] border border-slate-700 shadow-sm mt-6">
        ...
        <p className="text-sm text-slate-400 mb-4">Share your favorite sports club moments with the admin team! By uploading, you consent to sharing these specific photos.</p>
        ...
        {permissionsGranted ? (
           ...
           <div className="bg-amber-500/100/10 border border-amber-500/20 p-5 rounded-xl">
             ...
             }} className="bg-amber-500/100 text-white px-5 py-2.5 rounded-lg font-bold shadow-sm hover:bg-amber-600 transition-colors text-sm">
      ...
      <div>
        ...
        <div 
          ...
          className="bg-[#1e293b] border border-slate-700 shadow-sm p-5 rounded-2xl flex items-center justify-between cursor-pointer hover:border-[#2563eb]/50 transition-all mb-4"
        >
          <div className="flex items-center gap-4">
            <div className="text-2xl">👨‍🏫</div>
            <h4 className="font-bold text-lg text-white">Our Mentors</h4>
          </div>
        ...
        <div className="flex justify-between items-center mb-4 mt-6">
          ...
          {canEditHolidays && (
            <button onClick={handleAddDept} className="text-xs flex items-center gap-1 bg-slate-700 hover:bg-[#2563eb] hover:text-white px-3 py-1.5 rounded-lg transition-colors font-bold text-slate-300">
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4">
          {coreDepts.map(d => (
            <div 
              ...
              className="bg-[#1e293b] border border-slate-700 shadow-sm p-3 md:p-4 rounded-xl md:rounded-2xl flex flex-col items-center justify-center text-center gap-2 cursor-pointer hover:bg-slate-800 hover:border-[#2563eb]/50 transition-all group relative"
            >
              {canEditHolidays && (
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={(e) => handleEditDept(d, e)} className="p-1 bg-slate-700 rounded hover:bg-[#2563eb] hover:text-white text-slate-400 transition-colors">
                    <Edit2 size={10} />
                  </button>
                  <button onClick={(e) => handleDeleteDept(d, e)} className="p-1 bg-slate-700 rounded hover:bg-red-500/100 hover:text-white text-slate-400 transition-colors">
                    <Trash2 size={10} />
                  </button>
                </div>
              )}
...
function MentorsTab() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {mentors.map(m => (
          <div key={m.id} className="bg-[#1e293b] border border-slate-700 rounded-[24px] p-6 relative group hover:border-[#2563eb]/30 transition-all shadow-sm hover:shadow-md">
            ...
            {canEdit && (
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleEdit(m)} className="p-2 bg-slate-700 rounded-xl text-slate-400 hover:text-[#2563eb] transition-colors">
                  <Edit2 size={14} />
                </button>
                <button onClick={() => {
                  if(confirm("Delete this mentor?")) deleteMentor(m.id);
                }} className="p-2 bg-slate-700 rounded-xl text-slate-400 hover:text-red-500 transition-colors">
                  <Trash2 size={14} />
                </button>
              </div>
            )}
...
function CoreTab({ dept, setDept }: { dept: string, setDept: (d: string) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        {coreDepts.map(d => (
          <button 
            ...
            className={`whitespace-nowrap px-5 py-3 rounded-[16px] text-sm font-bold transition-all duration-300 border ${
              ...
              : 'bg-[#1e293b] text-slate-400 border-slate-700 hover:bg-slate-800 hover:text-white shadow-sm'
            }`}
          >
            {d.name}
          </button>
      ...
      <div className="bg-[#1e293b] border border-slate-700 rounded-[28px] p-6 min-h-[300px] shadow-sm">
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-600">
          <h3 className="text-xl font-bold text-white tracking-wide">{dept} <span className="text-[#2563eb]">Members</span></h3>
          {canEdit && (
            <button onClick={handleAdd} className="flex items-center gap-2 bg-[#10b981]/10 text-[#10b981] px-4 py-2 rounded-xl text-sm font-bold border border-[#10b981]/20 hover:bg-[#10b981]/20 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {members.map(m => (
            <div key={m.id} className="flex items-center justify-between p-5 bg-slate-800 rounded-[20px] border border-slate-700 group hover:border-[#2563eb]/30 hover:bg-[#1e293b] hover:shadow-sm transition-all">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 rounded-2xl bg-[#1e293b] border border-slate-700 flex items-center justify-center text-2xl shadow-sm text-[#2563eb]">
                  {getFaceEmoji(m.name)}
                </div>
                <div>
              ...
              {canEdit && (
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => handleEdit(m)} className="p-2.5 bg-[#1e293b] border border-slate-700 hover:bg-slate-700 rounded-xl text-slate-400 hover:text-[#2563eb] transition-colors">
                    <Edit2 size={16} />
                  </button>
                  <button onClick={() => {
                    if(confirm("Delete member?")) deleteCoreMember(m.id);
                  }} className="p-2.5 bg-[#1e293b] border border-slate-700 hover:bg-slate-700 rounded-xl text-slate-400 hover:text-red-500 transition-colors">
                    <Trash2 size={16} />
                  </button>
                </div>
              )}
