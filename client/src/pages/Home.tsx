...
import { Edit2, Trash2, PlusCircle, LayoutDashboard, Users, UserCog, ArrowRight } from 'lucide-react';
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      ...
      <div className="bg-gradient-to-br from-black/40 to-black/20 backdrop-blur-xl p-6 rounded-[28px] border border-white/10">
        ...
        <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
          {[1,2,3].map(i => (
            <div key={i} className="min-w-[180px] p-5 rounded-2xl bg-white/5 border border-white/10 shrink-0 hover:scale-105 transition-transform cursor-pointer">
              <p className="text-[10px] font-bold text-[#fca311] mb-2 tracking-wider">DEC 25 - JAN 1</p>
              <p className="font-bold text-lg leading-tight">Winter Break</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {mentors.length === 0 && (
          <div className="col-span-full text-center text-white/30 py-16 flex flex-col items-center gap-4 border border-dashed border-white/10 rounded-[24px]">
             <Users size={48} className="opacity-20" />
             <p className="font-medium tracking-wider uppercase text-sm">No mentors added yet</p>
          </div>
        )}
      </div>
    </motion.div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="bg-gradient-to-b from-white/10 to-white/5 border border-white/10 rounded-[28px] p-6 min-h-[300px] backdrop-blur-md">
        ...
        {members.length === 0 ? (
          <div className="text-center text-white/30 py-16 flex flex-col items-center gap-4">
            <Users size={48} className="opacity-20" />
            <p className="font-medium tracking-wider uppercase text-sm">No members assigned yet</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {members.map(m => (
              <div key={m.id} className="flex items-center justify-between p-5 bg-black/40 rounded-[20px] border border-white/5 group hover:border-[#6b5cff]/50 hover:bg-black/60 transition-all">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    {m.dateAdded && <p className="text-[10px] font-mono text-white/30 mt-2 uppercase tracking-widest">Added: {m.dateAdded}</p>}
                  </div>
                </div>
